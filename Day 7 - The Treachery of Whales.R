...
# Read the data
positions <- readLines("Input/day7.txt")


# Convert the input data to a numeric vector
positions <- as.numeric(strsplit(positions, split = ",")[[1]])
...
# ocean floor; sensors indicate a massive underground cave system just beyond 
...
min(rowSums(sapply(positions, function(x) abs(x - unique(positions)))))
## 323 647


# Which position has the minimum fuel consumption
positions[which(rowSums(sapply(1:1000, function(x) 
  abs(positions[x] - unique(positions)))) == 323647)]
## Position 97


#--- Part Two ----

# It turns out that crab submarine engines don't burn fuel at a constant rate. 
# Instead, each change of 1 step in horizontal position costs 1 more unit of fuel 
# than the last: the first step costs 1, the second step costs 2, 
# the third step costs 3, and so on


# Find the minimum fuel consumption with the new information
min(rowSums(
  sapply(positions, function(x) 
    sapply(min(positions):max(positions), function(y) sum(0:abs(x - y))))))
## 87 640 209
